...
const domainSchema = z.object({
  domainName: z.string().min(3).regex(/^[a-zA-Z0-9][-a-zA-Z0-9]*(\.[a-zA-Z0-9][-a-zA-Z0-9]*)+$/),
});
...
async function checkDomainDNS(domain: string): Promise<{
  ...
  try {
    ...
    if (!dnsData.Answer || dnsData.Answer.length === 0) {
      // 进一步检查NS记录，确认域名是否存在
      const nsResponse = await fetch(`https://cloudflare-dns.com/dns-query?name=${domain}&type=NS`, {
        headers: {
          'Accept': 'application/dns-json'
        }
      });
    ...
    const pointsToOurServer = dnsData.Answer.some(record => record.data === serverIP);
